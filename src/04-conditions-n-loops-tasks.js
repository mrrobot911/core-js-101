...
function doRectanglesOverlap(rect1, rect2) {
  return Math.min(rect1.width + rect1.left, rect2.width + rect2.left)
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  return ''.concat(isStartIncluded ? '[' : '(', a < b ? a : b, ', ', a > b ? a : b, isEndIncluded ? ']' : ')');
}
...
function isCreditCardNumber(ccn) {
  const value = String(ccn).split('').reverse().join('');
  let sum = 0;
...
function getDigitalRoot(num) {
  ...
  const resNum = Math.floor(num / 10);
  return getDigitalRoot(lastDigit + getDigitalRoot(resNum));
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (Object.keys(t).includes(str[i]) && t[str[i]] !== res[res.length - 1]) {
      res.push(str[i]);
    } else if (t[res[res.length - 1]] !== str[i]) {
      return false;
    } else {
      res.pop();
    }
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathes[0].length; i += 1) {
    if (pathes.every((it) => it[i] === pathes[0][i])) arr.push(pathes[0][i]);
    else break;
  }
